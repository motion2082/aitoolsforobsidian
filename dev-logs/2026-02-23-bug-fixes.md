...
**Fix:**
- **`installAgent()` early return**: Removed entirely. Always runs `npm install -g` — it's idempotent (fast for already-installed packages) and repairs broken installs from stale node_modules
- **`installAgent()` close handler**: Now calls `detectNodePath()` alongside `detectAgentPath()` — only reports success if both agent stub AND Node.js exist
- **`saveSettings()`**: Now calls `detectNodePath()` — only trusts detected agent path if Node.js is available; saves bare binary name otherwise
